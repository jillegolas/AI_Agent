...
def load_transactions(raw_rows=None):
...
def fraud_score(transaction, average_amount, trusted_countries=None):

    """Return a simple risk score from transaction attributes."""
    if trusted_countries is None:
        trusted_countries = []
    ...
    if transaction.get("device_id") == "unknown":
